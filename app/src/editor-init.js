...
require(['vs/editor/editor.main'], function () {
    ...
    // ── Color swatch decorations (PaletteColor / CopperColor) ─────────────────
    // Shows a colored square in the glyph margin next to every PaletteColor and
    // CopperColor line so the OCS color is visible at a glance.

    const _swatchStyles = new Map();   // cssColor → className

    function _ensureSwatchStyle(cssColor) {
        if (_swatchStyles.has(cssColor)) return _swatchStyles.get(cssColor);
        const key = cssColor.replace(/[^0-9,]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
        const cls = 'bsm-sw-' + key;
        const s   = document.createElement('style');
        s.textContent = `.monaco-editor .${cls} { background: ${cssColor}; border-radius: 2px; border: 1px solid rgba(255,255,255,0.15); }`;
        document.head.appendChild(s);
        _swatchStyles.set(cssColor, cls);
        return cls;
    }

    function _ocsToCSS(r, g, b) {
        r = Math.min(15, Math.max(0, r));
        g = Math.min(15, Math.max(0, g));
        b = Math.min(15, Math.max(0, b));
        return `rgb(${r * 17},${g * 17},${b * 17})`;
    }

    const _colorDecors = window._monacoEditor.createDecorationsCollection();
    let   _swatchTimer  = null;

    function _scheduleSwatchUpdate() {
        clearTimeout(_swatchTimer);
        _swatchTimer = setTimeout(_updateColorSwatches, 300);
    }

    function _updateColorSwatches() {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            let m = /^\s*PaletteColor\s+\d+\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/i.exec(line);
            if (m) {
                decorations.push({
                    range:   new monaco.Range(i + 1, 1, i + 1, 1),
                    options: { glyphMarginClassName: _ensureSwatchStyle(_ocsToCSS(+m[1], +m[2], +m[3])) },
                });
            }

            // CopperColor y, r, g, b
            m = /^\s*CopperColor\s+\d+\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/i.exec(line);
            if (m) {
                decorations.push({
                    range:   new monaco.Range(i + 1, 1, i + 1, 1),
                    options: { glyphMarginClassName: _ensureSwatchStyle(_ocsToCSS(+m[1], +m[2], +m[3])) },
        ...
        _colorDecors.set(decorations);
    }

    window._monacoEditor.onDidChangeModelContent(_scheduleSwatchUpdate);
